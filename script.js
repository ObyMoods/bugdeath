  ...
  uploadBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      result.innerHTML = `
        ...
        <img src="${url}" style="max-width:100%;margin-top:10px;border-radius:8px;">
      `;
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_KEY = "https://api.yydz.my.id/api/download/tiktok?url=https%3A%2F%2Fvt.tiktok.com%2FZSD2SoPvY%2F&apikey=API_YUDZ"; // Ganti dengan API key kamu
  const API_BASE = "https://api.yydz.my.id/api/download/tiktok";
      ...
      try {
        ...
        if (data && data.status === true && data.result && data.result.nowatermark) {
          ...
          tiktokResult.innerHTML = `
            ...
            <br>
            <a href="${videoURL}" download="tiktok_video.mp4" class="btn-primary" style="display:inline-block; padding:5px 10px; background:#4CAF50; color:#fff; text-decoration:none; border-radius:4px; margin-top:5px;">💾 Download Video</a>
          `;
        } else {
        ...
        console.error(err);
        tiktokMsg.textContent = "❌ Terjadi kesalahan saat memproses: " + err.message;
      }
    });
  }
});
