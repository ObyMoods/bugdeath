...
$("#premList").innerHTML = premium.length ? premium.map((u, i)=>{
  return `<div>
    <b>${u.id}</b> • ${u.days} hari (${u.info || "Premium"}) 
    <button class="item-btn del-prem" data-i="${i}">Hapus</button>
  </div>`;
...
app.post("/api/save-avatar", async (req, res) => {
  try {
    const url = `https://api.github.com/repos/${GITHUB_USER}/${GITHUB_REPO}/contents/avatar.json`;
    ...
    const putRes = await fetch(url, {
      ...
      body: JSON.stringify({
        message: "Update avatar.json via admin panel",
        content: Buffer.from(JSON.stringify(req.body, null, 2)).toString("base64"),
        sha: fileData.sha
      })
    });

    if (!putRes.ok) throw new Error("Gagal update avatar.json");
    res.json({ success: true });
  } catch (e) {
    console.error("save-avatar error:", e.message);
...
app.get("/api/avatar", async (req, res) => {
  try {
    const url = `https://api.github.com/repos/${GITHUB_USER}/${GITHUB_REPO}/contents/avatar.json?ts=${Date.now()}`;
    const ghRes = await fetch(url, { headers: { Accept: "application/vnd.github.v3+json" } });
    const data = await ghRes.json();
    const content = Buffer.from(data.content, "base64").toString("utf8");
    res.type("application/json").send(content);
  } catch (e) {
    console.error("get-avatar error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
async function saveAvatar(url) {
  try {
    ...
    if (!res.ok) throw new Error("Gagal simpan avatar");
    return true;
  } catch (e) {
...
async function loadAvatar() {
  try {
    ...
    if (!res.ok) throw new Error("Gagal load avatar.json");
    const data = await res.json();
    return data.url || "https://via.placeholder.com/120?text=Premium";
  } catch (e) {
    console.error("❌ loadAvatar error:", e.message);
    return "https://via.placeholder.com/120?text=Premium";
  }
}

btnUpdateAvatar.addEventListener("click", async () => {
  const url = adminAvatarUrl.value.trim();
  if (!url) {
    avatarMsgEl.innerHTML = `<span style="color:red">❌ Masukkan URL avatar!</span>`;
    return;
  }

  const ok = await saveAvatar(url);
  if (ok) {
    adminAvatarPreview.src = url;
    avatarMsgEl.innerHTML = `<span style="color:green">✅ Avatar berhasil diupdate!</span>`;
    setTimeout(() => (avatarMsgEl.innerHTML = ""), 3000);
  }
});