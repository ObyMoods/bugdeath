...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnUpload.addEventListener('click', async () => {
    ...
    formData.append('reqtype', 'fileupload'); // Wajib
    formData.append('userhash', '');          // Bisa kosong
    formData.append('fileToUpload', file);    // File dari input
    ...
    try {
      ...
      if (data.startsWith('http')) {
        ...
        result.innerHTML = `
          ...
          ${file.type.startsWith('image/') ? `<img src="${data}" />` : ''}
        `;
      } else {
  ...
  if (btnDownloadTikTok) {
    btnDownloadTikTok.addEventListener('click', async () => {
      ...
      try {
        ...
        if (data && data.result && data.result.nowatermark) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  obfTextBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      try {
        var result = obfuscateText(code);
        textOutput.value = result.invisible; // tampilkan invisible
        textOutput.dataset.raw = result.raw; // simpan versi asli
        textOutput.dataset.invisible = result.invisible; // simpan versi tampilan
  ...
  clearBtn.addEventListener("click", () => {
    ...
    textOutput.removeAttribute("data-invisible");
    textOutput.style.display = "none";
  ...
  obfFileBtn.addEventListener("click", async () => {
    ...
    setTimeout(() => {
      try {
        ...
        // ✅ Download hasil invisible (sama persis dengan yang tampil)
        var blob = new Blob([result.invisible], { type: "text/javascript" });
