...
$("#clearText")?.addEventListener("click", ()=>{ if(confirm("Hapus semua teks?")){ texts=[]; save("tb_texts", texts); renderAdmin(); } });
...
$("#clearStickers")?.addEventListener("click", ()=>{ if(confirm("Hapus semua sticker?")){ stickers=[]; save("tb_stickers", stickers); renderAdmin(); } });

// ===== Event Delegation for Delete Buttons =====
document.addEventListener("click", async (e)=>{
  if(e.target.matches(".del-text")){ texts.splice(parseInt(e.target.dataset.i),1); save("tb_texts", texts); renderAdmin(); }
  if(e.target.matches(".del-st")){ stickers.splice(parseInt(e.target.dataset.i),1); save("tb_stickers", stickers); renderAdmin(); }
  if(e.target.matches(".del-prem")){ premium.splice(parseInt(e.target.dataset.i),1); await savePremium(); renderAdmin(); }
  if(e.target.matches(".del-tok")){ 
    const tokens = loadTokens(sessionUser); tokens.splice(parseInt(e.target.dataset.i),1); saveTokens(sessionUser,tokens); renderTokens(sessionUser);
  }
});

// ===== Admin Add / Clear Premium =====
...
function renderTokens(uid){
  const tokens = loadTokens(uid); const list=$("#tokenList"); list.innerHTML="";
