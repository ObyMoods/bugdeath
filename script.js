...
// Tombol Catbox di menu lainnya
const btnShowCatbox = document.getElementById('btnShowCatbox');
if (btnShowCatbox) {
  btnShowCatbox.addEventListener('click', () => showPage('catboxPage'));
}

// Tombol kembali dari Catbox ke Menu Lainnya
const btnBackToMenu = document.getElementById('btnBackToMenu');
if (btnBackToMenu) {
  btnBackToMenu.addEventListener('click', () => showPage('menuLainnya'));
}
...
if (btnCatboxUpload) {
  btnCatboxUpload.addEventListener('click', async () => {
    ...
    if (!fileInput || !fileInput.files.length) {
      if (msg) msg.textContent = "❌ Pilih file terlebih dahulu!";
    ...
    if (msg) msg.textContent = "⏳ Mengupload...";
    if (result) result.innerHTML = "";
    ...
    try {
      ...
      if (data.startsWith('http')) {
        if (msg) msg.textContent = "✅ Upload berhasil!";
        if (result) result.innerHTML = `
          <a href="${data}" target="_blank">${data}</a><br>
          <img src="${data}" style="max-width:100%; margin-top:10px;">
        `;
      } else {
        if (msg) msg.textContent = "❌ Upload gagal!";
        if (result) result.textContent = data;
      }
    } catch (err) {
      if (msg) msg.textContent = "❌ Terjadi kesalahan saat upload!";
      console.error(err);
    }
  });
}
