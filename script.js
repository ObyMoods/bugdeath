...
// ===== Reset Teks / Sticker / Token di Dashboard =====
$("#resetTexts")?.addEventListener("click", ()=>{
  if(confirm("⚠️ Hapus semua teks?")){
    texts=[]; saveTexts(); // otomatis render list & update dropdown
    alert("✅ Semua teks telah dihapus!");
  }
});

$("#resetStickers")?.addEventListener("click", ()=>{
  if(confirm("⚠️ Hapus semua sticker?")){
    stickers=[]; saveStickers(); // otomatis render list & update dropdown
    alert("✅ Semua sticker telah dihapus!");
  }
});

$("#resetTokens")?.addEventListener("click", ()=>{
  if(confirm("⚠️ Hapus semua token Anda?")){
    saveTokens(sessionUser, []); // otomatis render token list
    alert("✅ Semua token telah dihapus!");
  }
});
...
// Order Nokos
document.getElementById("btnOrderNokos").addEventListener("click", async ()=>{
  const service = document.getElementById("nokosService").value.trim();
  const country = document.getElementById("nokosCountry").value.trim();
  const res = await fetch("/api/order-nokos", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({service,country})});
  const data = await res.json();
  document.getElementById("nokosResult").textContent = JSON.stringify(data,null,2);
});
...
document.getElementById("btnDeposit").addEventListener("click", async ()=>{
  ...
  const data = await res.json();
  document.getElementById("depositResult").textContent = JSON.stringify(data,null,2);
  if(data.qris){
    const img = new Image();
    img.src = "data:image/png;base64,"+data.qris;
    document.getElementById("depositResult").appendChild(img);
  }
});

// Cek deposit
document.getElementById("btnCekDeposit").addEventListener("click", async ()=>{
  const id = document.getElementById("depositId").value.trim();
  const res = await fetch("/api/cek-deposit/"+id);
  const data = await res.json();
  document.getElementById("cekDepositResult").textContent = JSON.stringify(data,null,2);
});
...
document.getElementById("btnSaldo").addEventListener("click", async ()=>{
  ...
  const data = await res.json();
  document.getElementById("saldoResult").textContent = JSON.stringify(data,null,2);
});