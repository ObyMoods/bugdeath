...
// ===== Add / Clear Text =====
// Simpan permanen texts dan stickers
...
document.addEventListener("click", (e) => {
  if(e.target.matches(".del-text")){
    ...
    texts.splice(idx, 1);
    localStorage.setItem("tb_texts", JSON.stringify(texts)); // simpan permanen
    renderAdmin();
  } else if(e.target.matches(".del-st")){
    const idx = parseInt(e.target.dataset.i);
    stickers.splice(idx, 1);
    localStorage.setItem("tb_stickers", JSON.stringify(stickers)); // simpan permanen
    renderAdmin();
  }
});
