...
document.getElementById('btnBackToMenu')?.addEventListener('click', () => {
  ...
  // Catbox Upload
  const btnCatboxUpload = document.getElementById('btnCatboxUpload');
  const catboxFile = document.getElementById('catboxFile');
  const catboxMsg = document.getElementById('catboxMsg');
  const catboxResult = document.getElementById('catboxResult');

  btnCatboxUpload?.addEventListener('click', async () => {
    if(!catboxFile.files.length){
      catboxMsg.textContent = "❌ Pilih foto terlebih dahulu!";
      catboxResult.innerHTML = "";
      return;
    }

    const file = catboxFile.files[0];
    if(!file.type.startsWith('image/')){
      catboxMsg.textContent = "❌ File harus berupa foto!";
      catboxResult.innerHTML = "";
      return;
    }
    ...
    catboxMsg.textContent = "⏳ Mengupload...";
    catboxResult.innerHTML = "";
    ...
    try{
      ...
      const data = await res.json();
      if(data.url){
        catboxMsg.textContent = "✅ Upload berhasil!";
        catboxResult.innerHTML = `<a href="${data.url}" target="_blank">${data.url}</a><br><img src="${data.url}" style="max-width:100%; margin-top:10px;">`;
      } else {
        catboxMsg.textContent = "❌ Upload gagal!";
        catboxResult.textContent = data.error || 'Terjadi kesalahan';
      }
    } catch(err){
      console.error(err);
      catboxMsg.textContent = "❌ Terjadi kesalahan saat upload!";
    }
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnShowTikTok')?.addEventListener('click', () => showPage('tiktokPage'));
  document.getElementById('btnBackMenuTikTok')?.addEventListener('click', () => showPage('menuLainnya'));
  document.getElementById('btnShowCatbox')?.addEventListener('click', () => showPage('catboxPage'));
  document.getElementById('btnBackToMenu')?.addEventListener('click', () => showPage('menuLainnya'));
    ...
    try {
      ...
      if(data.videoURL){
        ...
        tiktokResult.innerHTML = `
          ...
          <br>
          <a href="${data.videoURL}" download="tiktok_video.mp4" class="btn primary" style="margin-top:5px;">💾 Download Video</a>
        `;
      } else {
        tiktokMsg.textContent = "❌ " + (data.error || 'Gagal download TikTok');
      }
    } catch(err){
      console.error(err);
