...
$("#btnLogin").addEventListener("click", async () => {
  ...
  if (!p) {
    $("#loginMsg").textContent = "❌ Bukan user premium / expired";
    return;
  }
  sessionUser = id;
  showDashboard(id, p);
...
function showDashboard(id, user){
  ...
  const left = Math.max(0, Math.floor((user.expire - Date.now())/86400000));
  $("#premInfo").textContent = `${user.info || "Premium"} • Sisa ${left} hari • ID: ${id}`;
  renderTokens(id);
}
...
(function init(){
  ...
  if(su){
    loadPremium().then(()=>{
      const p = premium.find(u => u.id === su && u.expire > Date.now());
