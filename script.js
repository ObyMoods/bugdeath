...
function renderPremium() {
  const list = document.getElementById("premList");
  list.innerHTML = "";

  if (premium.length === 0) {
    list.innerHTML = `<p style="color:gray">Belum ada user premium.</p>`;
    return;
  }
  ...
  premium.forEach((u, i) => {
    ...
    li.innerHTML = `
      <div class="prem-item">
        <span class="prem-name">👤 ${u.info || "Tanpa Nama"}</span>
        <span class="prem-id">🆔 ${u.id}</span>
        <span class="prem-days">⏳ ${u.days} hari</span>
        <button class="btn danger" data-index="${i}">❌ Hapus</button>
      </div>
    `;
    ul.appendChild(li);
  });
