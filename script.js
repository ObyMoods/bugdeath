...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnUpload.addEventListener("click", async () => {
    ...
    if (!file.type.startsWith("image/")) {
      msg.textContent = "❌ Hanya file gambar (jpg/png/gif) yang diperbolehkan!";
      return;
    }
    ...
    msg.textContent = "⏳ Mengupload ke Catbox...";
    ...
    try {
      ...
      console.log("Catbox response:", data);

      if (data.startsWith("http")) {
        msg.textContent = "✅ Upload berhasil!";
        result.innerHTML = `
          <a href="${data}" target="_blank">${data}</a>
          <button id="copyBtn" style="margin-top:8px;">📋 Copy Link</button>
          <div style="margin-top:10px;">
            <img src="${data}" alt="Preview" style="max-width:300px; border-radius:8px;" />
          </div>
        `;
