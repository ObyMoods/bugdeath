...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function obfuscateText(code) {
    ...
    // return keduanya: versi asli & versi tampilan
    return {
      raw: obf,
  ...
  obfTextBtn.addEventListener("click", () => {
    ...
    setTimeout(() => {
      try {
        ...
        textOutput.value = result.invisible; // tampilkan yang invisible
        textOutput.dataset.raw = result.raw; // simpan versi asli di data attr
  ...
  obfFileBtn.addEventListener("click", async () => {
    ...
    setTimeout(() => {
      try {
        ...
        // Download pakai hasil raw asli (bukan ZWNJ)
        var blob = new Blob([result.raw], { type: "text/javascript" });
